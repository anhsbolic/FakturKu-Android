...
class InvoiceListAdapter(private val dataInvoiceList: List<Invoice>,
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        val status: String? = when (idStatus){
            0->{InvoiceStatus.DEBT.status}
            1->{InvoiceStatus.PAID.status}
            2->{InvoiceStatus.DRAFT.status}
            else->{null}
        }
        holder.txtStatus.text = status
        holder.txtId.text = dataInvoiceList[position].id
        holder.txtCreatedDate.text = dataInvoiceList[position].created_date
